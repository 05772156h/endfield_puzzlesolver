    ...
    def get_matrix_target():
        ...
        for direction in range(4 ** num_models):
                        ...
                        for i in range(m_choose[num]):
                            ...
                            for j in range(n_choose[num]):
                                ...
                                if matrix_choose[num][i, j] == 1:
                                    if target_filled_matrix[m_move + i, n_move + j] >= 1:
                                        move_able = False
                                        break
                        if move_able:
                            move_lists[num].append([m_move, n_move])

            # 得到总的可能的移动数目
            total_moves = 1
            total_move_able = True
            for num in range(num_models):
                if len(move_lists[num]) > 0:
                    total_moves *= len(move_lists[num])
                else:
                    total_move_able = False
                    break
            ...
            if total_move_able:
                ...
                move_choose_max = []
                for num in range(num_models):
                    move_choose.append(0)
                    move_choose_max.append(len(move_lists[num]))
                ...
                for move in range(total_moves):
                    ...
                    for num in range(num_models):
                        if not non_repeat:
                            break
                        if move_choose[num] >= move_choose_max[num]:
                            move_choose[num] = 0
                            move_choose[num + 1] += 1
                        ...
                        for i in range(m_choose[num]):
                            ...
                            for j in range(n_choose[num]):
                                ...
                                # 判断是否有重叠并加上新放置的电路模块
                                if matrix_choose[num][i, j] == 1:
                                    if target_filled_calc[move_lists[num][move_choose[num]][0] + i, move_lists[num][move_choose[num]][1] + j] >= 1:
                                        non_repeat = False
                                        break
                                    target_number_calc[move_lists[num][move_choose[num]][0] + i, move_lists[num][move_choose[num]][1] + j] += 1
                                    target_filled_calc[move_lists[num][move_choose[num]][0] + i, move_lists[num][move_choose[num]][1] + j] += 1
                        ...
                        if solved:
                            ...
                            for num in range(num_models):
                                for i in range(m_choose[num]):
                                    for j in range(n_choose[num]):
                                        if matrix_choose[num][i, j] == 1:
                                            entry_list[n * (move_lists[num][move_choose[num]][0] + i) + (move_lists[num][move_choose[num]][1] + j)]['background'] = colors[num]
                            end = time.time()
                            print('解谜时间为{}'.format(end - start))
